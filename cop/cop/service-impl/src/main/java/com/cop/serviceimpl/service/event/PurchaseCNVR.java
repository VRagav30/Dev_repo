...
public class PurchaseCNVR {
	...
	public Mmtotal generateMMTotal(Mmtotal mt) {
		...
		//BigDecimal[] stock;
		//List<BigDecimal[]> mtrList = mtr.findCstockbyPlantandstocktype(mt.getItemcode(), mt.getPlant(), mt.getDate(),	"O");
				BigDecimal[] stock;
		...
		for (Mmtotal mmtotal : mtrLst) {
			...
			mmtotal.setStockvalue(mmtotal.getStockvalue().add(mt.getStockvalue()));
	...
	public Mmtotal generateMMTotalContra(Mmtotal mt) {
		...
		//COSTACC SAME AS IN CNVR IN COSTTOTAL- 27TH DEC 2020
		...
		//List<BigDecimal[]> mtrList = mtr.findCstockbyItemcodePlantValsubBatch(mt.getItemcode(), mt.getPlant(),
			//	mt.getDate(), loh.getValsub(), mt.getBatchnumber(),mt.getSaleorder());
		...
		BigDecimal[] stock;
		  ...
		  mat.getBatchcosting().equalsIgnoreCase("NO")) mtrList =
		  mtr.findCstockStockwithoutBatch(mt.getItemcode(), mt.getPlant(),
		  mt.getDate(), mt.getStocktype(), loh.getValsub(), mt.getSaleorder()); else
		  mtrList = mtr.findCstockStockwithBatch(mt.getItemcode(), mt.getPlant(),
		  mt.getDate(), mt.getStocktype(), loh.getValsub(), mt.getBatchnumber(),
		  mt.getSaleorder());
