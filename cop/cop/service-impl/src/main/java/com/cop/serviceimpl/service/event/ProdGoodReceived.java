...
public class ProdGoodReceived {
	...
	public Mmtotal generateMMTotal(Mmtotal mt) {
		...
		System.out.println(mat.getOboind() + "");
		// List<Mrc> MrcList =
		// mrcr.findAllbyItemcodePlantDateMpricetyp(mt.getItemcode(), mt.getPlant(),
		// mt.getDate());
		// ORDAMOUNT
		// mt.setOrdamount(mt.getBuomqty().multiply(MrcList.get(0).getPrice()));
		...
		if ((mat.getMattyp() != ("SFG")) && (mat.getMattyp() != ("FG"))) {
			...
			System.out.println("OBOB INDCATOR");
		...
		if ((mat.getMattyp().equals("SFG") || mat.getMattyp().equals("FG"))) {
			System.out.println("prd inDIOCATOR");
		...
		System.out.println(mt.getOrdamount() + " searching param");
		...
		if (mtrList.isEmpty()) {
			...
			System.out.println(stockvalue + " Param 2");
