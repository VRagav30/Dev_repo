...
public class DNQR {
	...
	public Mmtotal generateMMTotal(Mmtotal mt) {
		...
		System.out.println("Entering Purchase DNQR");
		...
		// CSTOCK and stockvalue and ad indicator
	...
	public Mmtotal generateMMTotalContra(Mmtotal mt) {
		...
		List<Caval> cvList = cvr.findAllbyOrgnCodeMatValGrp("INV", loh.getOrgncode(), mat.getValgrp());
		...
		if (mtrList.isEmpty()) {
			...
			mt.setCstock(cstock);
			if (mt.getBuomqty().compareTo(cstock) == 1) {
				BigDecimal prdRatio = mt.getPrdamount().divide(mt.getPrdqty(), 2, RoundingMode.HALF_DOWN);
				stockValueCorrection = prdRatio.multiply(mt.getCstock());
				mt.setStockvalue(stockvalue.subtract(stockValueCorrection));
		...
		List<Mmtotal> mtrLst;
		if (mat.getBatch().equalsIgnoreCase("YES") && mat.getBatchcosting().equalsIgnoreCase("NO"))
			mtrLst = mtr.findallbyBatchcosting(mt.getItemcode(), loh.getPlant(), mt.getDate(), mt.getStocktype(),
					loh.getValsub(), mt.getSaleorder());
		else
			mtrLst = mtr.findCstockStockvaluebyGRDateandstocktype(mt.getItemcode(), loh.getPlant(), mt.getDate(),
					mt.getStocktype(), loh.getValsub(), mt.getBatchnumber(), mt.getSaleorder());
		for (Mmtotal mmtotal : mtrLst) {
