...
@Component
public class PlantToPlantTransfer {
	...
	public Mmtotal generateMMTotal(Mmtotal mt) {
		...
		// mt.setTplant(mt.getPlant());
		List<Material> matList = mr.findAllByMatlcodeandPlantValsub(mt.getItemcode(), mt.getPlant(), mt.getValsub());
		mat = matList.get(0);

		// ORDAMOUNT
		List<Mrc> MrcListStandard = mrcr.findAllbyMpricetypValsub(mt.getItemcode(), mt.getSplant(), mt.getDate(), "STD",
				mt.getValsub(), mt.getBatchnumber());

		List<Mrc> MrcListMoving = mrcr.findAllbyMpricetypValsub(mt.getItemcode(), mt.getSplant(), mt.getDate(),
				"MOVING", mt.getValsub(), mt.getBatchnumber());

		List<Mrc> MrcListPlan = mrcr.findAllbyMpricetypValsub(mt.getItemcode(), mt.getSplant(), mt.getDate(), "PLAN",
				mt.getValsub(), mt.getBatchnumber());
		...
		List<BigDecimal[]> mmtrList;
		if (mat.getBatch().equalsIgnoreCase("YES") && mat.getBatchcosting().equalsIgnoreCase("NO"))
			mmtrList = mmtr.findCstockStockwithoutBatch(mt.getItemcode(), mt.getPlant(), mt.getDate(),
					mt.getStocktype(), mt.getValsub(), mt.getSaleorder());
		else
			mmtrList = mmtr.findCstockStockwithBatch(mt.getItemcode(), mt.getPlant(), mt.getDate(), mt.getStocktype(),
	...
	public Mmtotal generateMMTotalContra(Mmtotal mt) {
		mt.setTrnsevent("PT-CONTRA");
		mt.setPlant(mt.getSplant());
