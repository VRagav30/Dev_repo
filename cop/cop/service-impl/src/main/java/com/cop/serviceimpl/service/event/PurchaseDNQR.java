...
public class PurchaseDNQR {
	...
	public Mmtotal generateMMTotal(Mmtotal mt) {
		...
		oh = ohList.get(0);
		System.out.println(oh.getObjcode() + "DNQR");
	...
	public Mmtotal generateMMTotalContra(Mmtotal mt) {
		...
		// COSTACC SAME AS IN dnqr IN COSTTOTAL - 27TH DEC 2020
		...
		List<Mmtotal> mtrLst = mtr.findCstockStockvaluebyGRDateandstocktype(loh.getItemcode(), loh.getPlant(),
				mt.getDate(), mt.getStocktype(), loh.getValsub(), mt.getBatchnumber(), mt.getSaleorder());
		for (Mmtotal mmtotal : mtrLst) {
