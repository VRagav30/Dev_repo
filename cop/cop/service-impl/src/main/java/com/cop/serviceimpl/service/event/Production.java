...
public class Production {
	...
	@Autowired
	private ProdGoodReceived pgr;
	@Autowired
	private ProdGoodIssued pgi;
	@Autowired
	private ProdByProduct pbp;
	@Autowired
	private ProdOperationRevaluation por;
	@Autowired
	private ProdOperationalCost poc;
	@Autowired
	...
	public void processProductionRecords(List<Transaction> productionRecords) {
		...
		while (itr.hasNext()) {
			...
			List<Costtotal> orTransactions=new ArrayList<>();
			...
			for (String str : tempMap.keySet())
				...
				case "GR":
					...
					break;
				case "BP":
					for (Transaction source : tempMap.get("BP")) {
						Mmtotal target = new Mmtotal();
						BeanUtils.copyProperties(source, target);
						bpTransactions.add(target);
					}
						break;
						
				case "OR":

					for (Transaction source : tempMap.get("OR")) {
						Costtotal target = new Costtotal();
						BeanUtils.copyProperties(source, target);
						orTransactions.add(target);
					}
					break;
				default:
			...
			if (grTransactionAutopost.size() > 0) {
				...
				for (Costtotal ct : ocTransactions) {
					...
					objItNum += 10;
					ctContra = poc.generateCostTotalContra(ctContra);
					ctr.save(ct);
					ctr.save(ctContra);
				}
				...
				for(Costtotal ct:orTransactions)
				{
					ct.setDocnumber(currentNum);
					List<Costtotal> orList=por.generateCostTotal(ct);
					System.out.println("Size is" + orList.size());
					for(Costtotal ct1:orList)
						
						{
						System.out.println("size from for loop"+orList.size());
						ct1.setDocitnum(new BigDecimal(objItNum));
							objItNum += 10;
							
						ctr.save(ct1);
						Costtotal ctContra = por.generateCostTotalContra(ct1);
						ctContra.setDocitnum(new BigDecimal(objItNum));
						objItNum += 10;
					    ctr.save(ctContra);
						}
